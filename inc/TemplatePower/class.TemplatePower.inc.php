...
class TemplatePower extends TemplatePowerParser
{
  ...
  /**********
	 ...
	 * @access public
     */
    public function printToScreen($menuTopo = true)
    {
        if ($menuTopo) {
            $this->assignGlobal('menu', montarMenu(' menuTopo = 1'));
        }

        if(is_numeric($_SESSION[SESSAO_SISTEMA]['idusuario'])){
            $this->newBlock('HabilitarMenuUsuario');
            $this->assignGlobal('titulo', TITULO_SISTEMA);
            $this->assignGlobal('idusuarioLogado', $_SESSION[SESSAO_SISTEMA]['idusuario']);
            $this->assignGlobal('nomeUsuarioLogado', $_SESSION[SESSAO_SISTEMA]['usuario']);
            $this->assignGlobal('logoSistemaInterno', LOGO_SISTEMA_INTERNO);
            $this->assignGlobal('anoCorrente', date('Y'));
