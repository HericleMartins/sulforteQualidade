...
class Spreadsheet_Excel_Reader {
	...
	function valDecode($row,$col,$sheet=0) {
		$col = $this->getCol($col);
		if (array_key_exists($row,$this->sheets[$sheet]['cells']) && array_key_exists($col,$this->sheets[$sheet]['cells'][$row])) {
			return html_entity_decode($this->sheets[$sheet]['cells'][$row][$col], ENT_QUOTES, 'ISO-8859-1');
		}
		return "";
	}
...
//				die('The filename ' . $sFileName . ' is not readable');
				throw new Exception('O arquivo ' . $sFileName . ' não pode ser lido');
			}
			...
			switch ($code) {
				case SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						$retstr = ($asciiEncoding) ? htmlentities($retstr, ENT_QUOTES, 'ISO-8859-1') : $this->_encodeUTF16($retstr);
